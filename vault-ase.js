  ...
  const DIGITAL_PRICE  = 197;
  const PHYSICAL_PRICE = 207;
  ...
  let state = {
    ...
    currentOdds: 10,
    boxNumber: 1,
    packResults: Array(20).fill(null),
    isRevealing: false,
    goldWon: false
  };
  ...
  function initSession() {
    ...
    const goldPosition = (simpleHashInt(combined) % 20) + 1; // 1-10
    ...
    state.spinCount   = 0;
    state.currentOdds = 20;
    state.packResults = Array(20).fill(null);
  ...
  function updateOddsUI() {
    ...
    document.getElementById('spins-label').textContent   = spins + ' of 20 opened';
  }
  ...
  async function updateCustomerPnL(result) {
    ...
    const GOLD_VALUE = 580; // retail value of Gold Eagle // retail value of gold bar
    ...
    try {
      ...
      const newGoldWins = (customer.total_gold_wins       || 0) + (result === "gold_eagle" ? 1 : 0);
      const newGoldVal  = (customer.total_gold_value      || 0) + (result === "gold_eagle" ? GOLD_VALUE : 0);
      ...
      await fetch(
        `${SUPABASE_URL}/rest/v1/vault_customers?email=eq.${encodeURIComponent(email)}`,
        {
          ...
          body: JSON.stringify({
            ...
            ...(result === "gold_eagle" ? { last_gold_win_at: new Date().toISOString() } : {})
          })
        }
      );
  ...
  async function openPack(packNum) {
    ...
    supaInsert('vault_orders', {
      ...
      buyback_offered: result === "gold_eagle"
    });
  ...
  async function runRevealAnimation(result) {
    ...
    // Set content
    if (result === "gold_eagle") {
      contImg.innerHTML   = '<img src="goldeagle.jpg" alt="1/10oz Gold Eagle MS69 NGC" style="width:100px;height:136px;object-fit:contain;filter:drop-shadow(0 4px 24px rgba(232,201,122,0.6));">';
      contImg.style.fontSize = 'inherit';
      contLabel.textContent = '1/10oz American Gold Eagle MS69';
      overlay.style.background = 'rgba(0,0,0,0.95)';
    } else {
      contImg.innerHTML   = '<img src="silvereagle.jpg" alt="Silver Eagle MS69 PCGS" style="width:110px;height:110px;object-fit:contain;border-radius:8px;filter:drop-shadow(0 4px 16px rgba(184,200,212,0.4));">';
      contImg.style.fontSize = 'inherit';
      contLabel.textContent = 'American Silver Eagle MS69';
    }
  ...
  function showWarNickelResult(result) {
    ...
    resultType.textContent = 'AMERICAN SILVER EAGLE MS69';
    resultDesc.innerHTML   = '<strong>PCGS or NGC Certified · MS69 or MS70 · Ships to your address on file.</strong>';

    const oddsRemaining = state.currentOdds;
    const spinsLeft     = 20 - state.spinCount;
  ...
  function startBuybackTimer() {
    ...
    buybackInterval = setInterval(() => {
      ...
      if (seconds <= 0) {
        ...
        alert('Offer expired — your Gold Eagle will ship automatically within 72 hours.');
      }
    }, 1000);
  }
  ...
  function chooseShip() {
    clearInterval(buybackInterval);
    closeModal('buyback-modal');
    alert('Your 1/10oz Gold Eagle is confirmed for shipment. You\'ll receive a tracking number within 3 business days. Congratulations! 🏆');
    if (state.session) { state.session._g = -1; }
    updateOddsUI(); renderPackGrid();
  }

  function chooseSellBack() {
    clearInterval(buybackInterval);
    closeModal('buyback-modal');
    alert('Sell-back confirmed. A check for $520.00 will be mailed to your address on file within 7 business days. Thank you! 🟡');
    updateOddsUI(); renderPackGrid();
  }
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      if (i === gold) {
        ...
        el.innerHTML = `
          ...
          <img src="goldeagle.jpg" alt="1/10oz Gold Eagle MS69" style="width:52px;height:70px;object-fit:contain;filter:drop-shadow(0 2px 16px rgba(232,201,122,0.7));position:absolute;top:50%;left:50%;transform:translate(-50%,-62%);" />
          <div class="pack-bottom-strip" style="background:rgba(232,201,122,0.25);border-top:1px solid rgba(232,201,122,0.4);">
            <div class="pack-label" style="color:var(--gold);">🏆 GOLD EAGLE WAS HERE</div>
  ...
  (function init() {
    ...
    // ── Auto-select from URL hash (e.g. vault-ase.html#digital) ──
